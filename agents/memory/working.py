...
from workflows.ontology import connect
...
def project_working_memory(dsn: str, run_id: str, recent_event_limit: int = 20) -> WorkingMemory | None:
    with connect(dsn) as conn:
        run_row = conn.execute("SELECT id, task_id, status FROM run WHERE id = %s", (run_id,)).fetchone()
        if run_row is None:
            return None
        ...
        if task_id is not None:
            task_row = conn.execute("SELECT goal_id, status FROM task WHERE id = %s", (task_id,)).fetchone()
            if task_row is not None:
                goal_id, task_status = task_row[0], task_row[1]
                goal_row = conn.execute("SELECT text, status FROM goal WHERE id = %s", (goal_id,)).fetchone()
                if goal_row is not None:
                    goal_text, goal_status = goal_row[0], goal_row[1]
            artifact_rows = conn.execute("SELECT id, uri, hash FROM artifact WHERE task_id = %s", (task_id,)).fetchall()
            open_artifacts = [{"id": r[0], "uri": r[1], "hash": r[2]} for r in artifact_rows]

        event_rows = conn.execute(
            "SELECT id, type, ts, payload FROM event WHERE run_id = %s ORDER BY ts DESC LIMIT %s",
        ...
        recent_events = [{"id": r[0], "type": r[1], "ts": r[2], "payload": r[3]} for r in event_rows]
