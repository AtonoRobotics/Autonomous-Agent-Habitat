...
**Nothing in this codebase creates an Operation for a real external call yet.** `daemon/operations` is real, generic, tested lifecycle machinery — the same honest gap `daemon/a2a`'s goal-dispatch and `daemon/selfimprove`'s optimizer both already document. No MCP tool call (`agents/harness/agentic_loop.py`), no `daemon/inference` call, no extension activation goes through `Propose`/`MarkDispatchPending`/`MarkDispatched` today; they all still complete or fail exactly as they did before this package existed, with no durable operation record and no crash recovery if the daemon dies mid-call. Wiring real call sites through this seam — deciding what `effect_type`/`owner_extension_id` an MCP call or an inference call should even declare, and what "the owning extension reconciles" concretely means for a call that has no extension backing it (a bare MCP server, say) — is genuinely separate, follow-on work, not a small addition to this PR. This package's own tests (and a real CLI-level crash-recovery smoke test) prove the mechanism works; nothing yet proves it's used. Two more narrower, accepted gaps: this package tracks exactly one attempt per `operation_id` (`contracts/effect-record.schema.json`'s `sequence`/`attempt` fields for multi-attempt retry orchestration aren't populated), and `MarkDispatchPending`'s policy-decision-consume and its own state update are two separate transactions — a crash in the narrow window between them leaves an effect visibly stuck `admitted` with an already-consumed decision, inspectable by an operator, not silently corrupted, but not fully atomic either.
