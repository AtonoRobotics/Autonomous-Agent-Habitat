...
**Backup/restore is one slice of a five-item list, deliberately.** §14 groups "backup/restore, upgrade/rollback, corruption recovery, resource exhaustion, and soak acceptance" as one line item; only backup/restore is built. Upgrade/rollback (schema down-migrations — `daemon/store`'s migration runner is forward-only today), corruption recovery, resource-exhaustion handling, and soak acceptance testing remain undone, not silently assumed covered by having backup/restore. `daemon/backup` also requires `pg_dump`/`pg_restore` to be present on the daemon's `PATH` — the same PostgreSQL client tools any Postgres deployment already installs alongside the server, not an extra dependency this repository introduces, but a real deployment prerequisite worth stating rather than leaving implicit.
