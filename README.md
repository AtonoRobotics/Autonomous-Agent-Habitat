...
- **Self-improvement candidate lifecycle (`daemon/selfimprove`).** `GENERATED -> EVALUATED -> CANARY -> PROMOTED | REJECTED; PROMOTED -> DEMOTED -> ROLLED_BACK`, for five candidate classes (prompt, retrieval policy, skill, module, core code). `RecordEval` takes only raw per-case pass/fail results — the daemon computes the pass/fail verdict itself against a fixed threshold, never a caller-declared verdict, the same "core decides from a declared property" discipline `daemon/policy` applies to reversibility. `Promote` requires a passing eval recorded strictly after entering canary (not merely a historical pass), demotes whatever else of the same class was previously promoted, and records that as the new candidate's rollback target so `Rollback` can restore the prior binding — failing closed if a third candidate has since been promoted over both. Canary/Promote/Demote/Rollback/Reject are all operator-only.
