...
@DBOS.step()
def request_approval(daemon_api_base_url: str, action: dict, risk: str) -> str:
...
def is_approved(daemon_api_base_url: str, ticket_id: str) -> bool:
...
def wait_for_approval(daemon_api_base_url: str, ticket_id: str, timeout_s: float = 300, poll_interval_s: float = 2) -> None:
    ...
    while time.monotonic() < deadline:
        if is_approved(daemon_api_base_url, ticket_id):
            return
