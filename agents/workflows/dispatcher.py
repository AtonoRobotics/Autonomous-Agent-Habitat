...
def claim_and_dispatch_once(db_path: str, daemon_grpc_addr: str, agent_token: str) -> list[str]:
    ...
    for goal_id, goal_text in open_goals:
        ...
        with SetWorkflowID(workflow_id):
            DBOS.start_workflow(pursue_goal, goal_id, goal_text, db_path, daemon_grpc_addr, agent_token)
...
def run_forever(db_path: str, daemon_grpc_addr: str, agent_token: str, poll_interval_sec: float = 2.0) -> None:
    ...
    try:
        while True:
            dispatched = claim_and_dispatch_once(db_path, daemon_grpc_addr, agent_token)
            if dispatched:
